...
/* ── NodeBox — fixed overlap by using minWidth:0 + limited label width ── */
const NodeBox = ({ nodeKey, status, isSubgraph = false }) => {
  ...
  return (
    ...
    >
      ...
      <div style={{
        width: 34, height: 34, borderRadius: 9, background: 'var(--surface-0)',
      ...
      {/* Label — takes remaining space, clips if too narrow */}
      ...
      {/* Badge — never shrinks, positioned at far right */}
      <div style={{ flexShrink: 0, marginLeft: 4 }}>
        {isCompleted && <span className="badge badge-green"><CheckCircle2 size={10} /> Done</span>}
        {isActive    && <span className="badge badge-blue"><Loader2 size={10} className="animate-spin-slow" /> Running</span>}
        {isFailed    && <span className="badge badge-red"><AlertCircle size={10} /> Failed</span>}
      </div>
    </div>
  );
};
  ...
  return (
    <div className="card" style={{ padding: '24px 20px', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        {/* Ingest */}
        <div style={{ width: '100%' }}>
          <NodeBox nodeKey="ingest" status={s('ingest')} />
        </div>
        ...
        {/* Parallel branches — key fix: gap reduced, minWidth:0 on each wrapper */}
        <div style={{ display: 'flex', gap: 8, width: '100%' }}>
          {['tech_discover', 'extract_business', 'discover_components'].map(key => (
        ...
        {/* Migrate subgraph */}
        <div style={{ width: '100%' }}>
          <NodeBox nodeKey="migrate" status={s('migrate')} isSubgraph />
        </div>
        ...
        {/* Assemble */}
        <div style={{ width: '100%' }}>
          <NodeBox nodeKey="assemble" status={s('assemble')} />
        </div>
